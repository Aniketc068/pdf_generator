...
def generate_pdfs():
    ...
    def generate_pdf_thread():
        ...
        if stop_flag.is_set():
            ...
            messagebox.showinfo(
                "Stopped",
                f"PDF generation was stopped by the user.\nTotal PDFs generated: {generated_count}"
            )
        else:
            # ✅ Successfully completed
            messagebox.showinfo(
                "Success",
                f"{generated_count} PDFs generated successfully!\n"
                f"Time taken: {minutes}m:{seconds}s:{milliseconds}ms"
            )
